...
import (
	...
	"github.com/go-lynx/lynx/plugins/internal/token/conf"
)
...
func (t *PlugToken) Load(b config.Value) (plugins.Plugin, error) {
...
func Token(token ...LoaderToken) plugins.Plugin {
	return &PlugToken{
		weight: 0,
		load:   token,
		conf:   &conf.Token{},
