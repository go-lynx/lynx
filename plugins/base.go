...
// TypedBasePlugin 泛型基础插件，提供类型安全的插件基础实现
...
// NewTypedBasePlugin 使用提供的元数据创建一个新的 TypedBasePlugin 实例。
// 这是初始化新类型安全插件实现的推荐方式。
...
// BasePlugin 保持向后兼容的基础插件
type BasePlugin = TypedBasePlugin[any]

// NewBasePlugin 创建基础插件（向后兼容）
