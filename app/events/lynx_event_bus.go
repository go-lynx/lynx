...
func (b *LynxEventBus) Subscribe(handler func(LynxEvent)) context.CancelFunc {
    if b.isClosed.Load() { return func() {} }
...
func (b *LynxEventBus) SubscribeTo(eventType EventType, handler func(LynxEvent)) context.CancelFunc {
    ...
    b.subscriberCount.Add(1)
    b.mu.Lock(); b.typeSubs[eventType]++; b.mu.Unlock()
    return func() {
        cancel()
        b.subscriberCount.Add(-1)
        b.mu.Lock(); if b.typeSubs[eventType] > 0 { b.typeSubs[eventType]-- }; b.mu.Unlock()
    }
}
...
func (b *LynxEventBus) SubscribeWithFilter(filter func(LynxEvent) bool, handler func(LynxEvent)) context.CancelFunc {
    ...
    wrapped := b.wrapHandler(func(ev LynxEvent) {
        if filter == nil || filter(ev) { handler(ev) }
    })
    cancel := kelindarEvent.Subscribe(b.dispatcher, wrapped)
    b.subscriberCount.Add(1)
...
func (b *LynxEventBus) SubscribeToWithFilter(eventType EventType, filter func(LynxEvent) bool, handler func(LynxEvent)) context.CancelFunc {
    ...
    b.mu.Lock(); b.typeSubs[eventType]++; b.mu.Unlock()
    return func() {
        cancel(); b.subscriberCount.Add(-1); b.mu.Lock(); if b.typeSubs[eventType] > 0 { b.typeSubs[eventType]-- }; b.mu.Unlock()
    }
}
...
func (b *LynxEventBus) Close() error {
    if b.isClosed.CompareAndSwap(false, true) {
        close(b.done)
        b.wg.Wait()
        if b.workerPool != nil { b.workerPool.Release() }
...
func (b *LynxEventBus) IsHealthy() bool {
    if b.isClosed.Load() || b.paused.Load() || b.isDegraded.Load() { return false }
    totalCap := b.totalQueueCap(); if totalCap > 0 { if b.totalQueueSize()*100/totalCap >= 80 { return false } }
    return true
}
...
func (b *LynxEventBus) GetSubscriberCount(eventType EventType) int {
    b.mu.RLock(); defer b.mu.RUnlock(); return b.typeSubs[eventType]
}
...
func (b *LynxEventBus) Pause() {
    ...
    if !b.paused.Load() {
        b.pauseStartTime = time.Now(); b.paused.Store(true); b.pauseCount.Add(1)
        ...
        if manager := GetGlobalEventBus(); manager != nil { _ = manager.PublishEvent(pauseEvent) }
    }
}
...
func (b *LynxEventBus) Resume() {
    b.mu.Lock(); defer b.mu.Unlock()
    if b.paused.Load() { b.pauseDuration += time.Since(b.pauseStartTime); b.paused.Store(false)
        ...
        if manager := GetGlobalEventBus(); manager != nil { _ = manager.PublishEvent(resumeEvent) }
    }
}
...
func (b *LynxEventBus) GetPauseStats() (time.Duration, time.Time) {
    ...
    if b.paused.Load() { return b.pauseDuration + time.Since(b.pauseStartTime), b.pauseStartTime }
...
func (b *LynxEventBus) GetWorkerPoolStats() (cap int, running int, free int, waiting int) {
    b.mu.RLock(); pool := b.workerPool; b.mu.RUnlock(); if pool == nil { return 0,0,0,0 }
...
func (b *LynxEventBus) GetEventHistory(filter *EventFilter) []LynxEvent {
    if b.history == nil { return []LynxEvent{} }
    if filter == nil { return b.history.GetEvents() }
...
func (b *LynxEventBus) GetPluginEventHistory(pluginID string, filter *EventFilter) []LynxEvent {
    ...
    if filter != nil {
        if len(filter.EventTypes) > 0 { pluginFilter.EventTypes = filter.EventTypes }
        if len(filter.Priorities) > 0 { pluginFilter.Priorities = filter.Priorities }
        if len(filter.Categories) > 0 { pluginFilter.Categories = filter.Categories }
        if filter.FromTime > 0 { pluginFilter.FromTime = filter.FromTime }
        if filter.ToTime > 0 { pluginFilter.ToTime = filter.ToTime }
        if len(filter.Metadata) > 0 { pluginFilter.Metadata = filter.Metadata }
        pluginFilter.HasError = filter.HasError
        if len(filter.Statuses) > 0 { pluginFilter.Statuses = filter.Statuses }
    }
...
func (b *LynxEventBus) run() {
    ...
    defer ticker.Stop(); defer b.wg.Done()
    for {
        select {
        case <-b.done:
            ...
            for time.Now().Before(drainDeadline) {
                ...
                for len(buf) < bs {
                    if ev, ok := b.tryRecvShared(); ok { buf = append(buf, ev) } else { break }
                }
                if len(buf) > 0 { b.publishBatch(buf); drained = true }
                b.bufferPool.Put(buf); GetGlobalMonitor().UpdateQueueSize(b.totalQueueSize())
                if !drained { return }
            }
            return
        case <-ticker.C:
        ...
        default:
            if b.paused.Load() { time.Sleep(10 * time.Millisecond); continue }
            batchSize := max(b.config.BatchSize, 1)
            buf := b.bufferPool.GetWithCapacity(batchSize)
            for len(buf) < batchSize {
                if ev, ok := b.tryRecvShared(); ok { buf = append(buf, ev) } else { break }
            }
            if len(buf) > 0 { ordered := orderByWeightedPriority(buf); b.publishBatch(ordered) }
            ...
            case ev := <-b.queue:
                ...
                for len(buf) < max(b.config.BatchSize, 1) {
                    if ev2, ok := b.tryRecvShared(); ok { buf = append(buf, ev2) } else { break }
                }
...
func orderByWeightedPriority(in []LynxEvent) []LynxEvent {
    if len(in) <= 1 { return in }
    var crit, high, norm, low []LynxEvent
    for i := range in {
        switch in[i].Priority {
        case PriorityCritical: crit = append(crit, in[i])
        case PriorityHigh: high = append(high, in[i])
        case PriorityLow: low = append(low, in[i])
        default: norm = append(norm, in[i])
        }
    }
    ...
    for ic < len(crit) || ih < len(high) || inx < len(norm) || il < len(low) {
        for k := 0; k < wc && ic < len(crit); k++ { out = append(out, crit[ic]); ic++ }
        for k := 0; k < wh && ih < len(high); k++ { out = append(out, high[ih]); ih++ }
        for k := 0; k < wn && inx < len(norm); k++ { out = append(out, norm[inx]); inx++ }
        for k := 0; k < wl && il < len(low); k++ { out = append(out, low[il]); il++ }
        if wc > 1 { wc-- }
        if wh > 1 { wh-- }
    }
    return out
...
func (b *LynxEventBus) UpdateConfig(cfg BusConfig) {
    ...
    if cfg.DropPolicy != "" { b.config.DropPolicy = cfg.DropPolicy }
    if cfg.ReserveForCritical >= 0 { b.config.ReserveForCritical = cfg.ReserveForCritical }
    if cfg.EnqueueBlockTimeout >= 0 { b.config.EnqueueBlockTimeout = cfg.EnqueueBlockTimeout }
}
